...
void pee_mutation( int* genome )
{
   ...
   if (RNG::Probability(2./3.)) // Bit (allele) mutation
   {
      ...
      */
      ...
      std::cerr << std::endl;
      */
      //memmove(genome+start*sizeof(int), genome+(end)*sizeof(int), (data.number_of_bits-end)*sizeof(int));
      for(int i=0; i<data.number_of_bits-end; ++i)
         genome[i+start] = genome[i+end];
      /*
      for(int i=0; i<data.number_of_bits; ++i)
         std::cerr << genome[i];
      std::cerr << std::endl;
